 ...
 * {@link Style} object.
 * <p>
 * TODO migrate to Puzzles Lib class
 */
@Deprecated
public final class ComponentHelper {
    ...
    public static Component getAsComponent(FormattedText formattedText) {
        return iterate(formattedText, FormattedContentSink::getAsComponent);
    }
    ...
    public static String getAsString(FormattedText formattedText) {
        return iterate(formattedText, FormattedContentSink::getAsString);
    }
    ...
    public static Component getAsComponent(FormattedCharSequence formattedCharSequence) {
        return iterate(formattedCharSequence, FormattedContentSink::getAsComponent);
    }
    ...
    public static String getAsString(FormattedCharSequence formattedCharSequence) {
        return iterate(formattedCharSequence, FormattedContentSink::getAsString);
    }

    private static <T> T iterate(FormattedText formattedText, FormattedContentSink.FormattedContentComposer<T> formattedContentComposer) {
        Objects.requireNonNull(formattedText, "formatted text is null");
        FormattedContentSink formattedContentSink = new FormattedContentSink();
        // use this to properly convert legacy formatting codes that are part of the string value
        StringDecomposer.iterateFormatted(formattedText, Style.EMPTY, formattedContentSink);
        return formattedContentComposer.apply(formattedContentSink);
    }

    private static <T> T iterate(FormattedCharSequence formattedCharSequence, FormattedContentSink.FormattedContentComposer<T> formattedContentComposer) {
        Objects.requireNonNull(formattedCharSequence, "formatted char sequence is null");
        FormattedContentSink formattedContentSink = new FormattedContentSink();
        formattedCharSequence.accept(formattedContentSink);
        // we have to convert to a component to be able to iterate using StringDecomposer::iterateFormatted
        return iterate(formattedContentSink.getAsComponent(), formattedContentComposer);
    }
