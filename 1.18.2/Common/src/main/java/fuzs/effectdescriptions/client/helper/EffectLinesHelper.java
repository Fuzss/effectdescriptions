...
public class EffectLinesHelper {
    ...
    public static void tryAddDisplayName(List<Component> lines, MobEffectInstance mobEffectInstance, boolean smallWidgets) {
        ...
        if (smallWidgets || EffectDescriptions.CONFIG.get(ClientConfig.class).alwaysAddEffectNameToTooltips) {
            ...
            Component durationComponent = new TextComponent("(").append(MobEffectUtil.formatDuration(mobEffectInstance, 1.0F)).append(")");
    ...
    public static void tryAddAttributes(List<Component> lines, MobEffectInstance mobEffectInstance) {
        ...
        if (!attributes.isEmpty()) {
            ...
            lines.add(new TranslatableComponent("potion.whenDrank").withStyle(ChatFormatting.DARK_PURPLE));
            for (Pair<Attribute, AttributeModifier> pair : attributes) {
                ...
                if (d0 > 0.0D) {
                    lines.add(new TranslatableComponent("attribute.modifier.plus." + attributeModifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslatableComponent(pair.getFirst().getDescriptionId())).withStyle(ChatFormatting.BLUE));
                } else if (d0 < 0.0D) {
                    d1 *= -1.0D;
                    lines.add(new TranslatableComponent("attribute.modifier.take." + attributeModifier.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslatableComponent(pair.getFirst().getDescriptionId())).withStyle(ChatFormatting.RED));
                }
            }
        }
    }
    ...
    public static void tryAddModName(List<Component> lines, MobEffectInstance mobEffectInstance) {
        ...
        if (EffectDescriptions.CONFIG.get(ClientConfig.class).addModNameToWidgetTooltips) {
            ...
            ModLoaderEnvironment.INSTANCE.getModName(Registry.MOB_EFFECT.getKey(mobEffectInstance.getEffect()).getNamespace())
    ...
    public static void tryAddInternalName(List<Component> lines, MobEffectInstance mobEffectInstance) {
        ...
        if (EffectDescriptions.CONFIG.get(ClientConfig.class).addInternalIdToWidgetTooltips) {
            ...
            lines.add(new TextComponent(Registry.MOB_EFFECT.getKey(mobEffectInstance.getEffect()).toString()).withStyle(ChatFormatting.DARK_GRAY));
        }
    }
}
